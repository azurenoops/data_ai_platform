locals {
  ...
  names = {
    ...
    portal_identity       = "${local.abbrs.managedIdentityUserAssignedIdentities}-portal-${local.resource_token}"
    ...
    cosmos_account        = "${local.abbrs.documentDB}-${local.resource_token}"
    ...
    portal_app_service    = "${local.abbrs.sites}-portal-${local.resource_token}"
